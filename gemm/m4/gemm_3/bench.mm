...
struct Kernel { const char* name; const char* file; const char* func; int tileM, tileN, tgSize; };
...
int main() {
    @autoreleasepool {
        ...
        Kernel kernels[] = {
            {"tg128_wide",  "config_a.metal", "gemm_tg128",      32, 128, 128},
            {"tg256_wide",  "config_b.metal", "gemm_tg256",      32, 128, 256},
            {"tg128_small", "config_c.metal", "gemm_small_tile", 16,  64, 128},
        };

        struct Shape { uint32_t M, N, K; };
        Shape shapes[] = {
            {256, 256, 128}, {256, 256, 512},
        ...
        fprintf(csv, "config,M,N,K,tgroups,time_ms,gflops,pct_peak\n");

        printf("gemm_3: Core saturation (threadgroup size / tile shape) — %s\n", dev.name.UTF8String);
        printf("=========================================================\n\n");

        for (auto& k : kernels) {
            auto p = compile(k.file, k.func);
            printf("--- %s (tile %dx%d, tg=%d) ---\n", k.name, k.tileM, k.tileN, k.tgSize);
            printf("  %5s %5s %5s  %5s  %8s  %8s  %6s\n", "M", "N", "K", "TGs", "ms", "GFLOPS", "peak%");
            ...
            for (int s = 0; s < nShapes; s++) {
                ...
                }, MTLSizeMake(gx, gy, 1), MTLSizeMake(k.tgSize, 1, 1));

                double flops = 2.0 * sh.M * sh.N * sh.K;
                double gflops = flops / t / 1e9;
                double pct = gflops / 5170.0 * 100;
                printf("  %5u %5u %5u  %5u  %8.3f  %8.1f  %5.1f%%\n", sh.M, sh.N, sh.K, tgs, t*1000, gflops, pct);
                fprintf(csv, "%s,%u,%u,%u,%u,%.4f,%.2f,%.1f\n", k.name, sh.M, sh.N, sh.K, tgs, t*1000, gflops, pct);
            }
            printf("\n");
        }
