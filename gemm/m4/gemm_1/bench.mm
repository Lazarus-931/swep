...
static id<MTLComputePipelineState> compile(const char* file, const char* func) {
    ...
    NSString* path = [cwd stringByAppendingPathComponent:[NSString stringWithUTF8String:file]];
    ...
    if (!fn) { fprintf(stderr, "Missing %s in %s\n", func, file); exit(1); }
...
int main() {
    @autoreleasepool {
        ...
        Kernel kernels[] = {
            {"config_a", "config_a.metal", "gemm_config_a", 48, 64},
            {"config_b", "config_b.metal", "gemm_config_b", 32, 64},
            {"config_c", "config_c.metal", "gemm_config_c", 32, 128},
        };
        ...
        printf("gemm_1: M>=256, N>=256, K>64 — %s\n", dev.name.UTF8String);
        printf("=========================================\n\n");

        for (auto& k : kernels) {
            auto p = compile(k.file, k.func);
            printf("--- %s (tile %dx%d) ---\n", k.name, k.tileM, k.tileN);
