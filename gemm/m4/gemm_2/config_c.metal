...
// Config C: K-block 128 (very deep)

#include <metal_stdlib>
using namespace metal;

kernel void gemm_k128(
...
{
    threadgroup float As[32][129];
    threadgroup float Bs[128][129];
    ...
    for (uint kb = 0; kb < K; kb += 128) {
        for (uint i = lid; i < 32 * 128; i += 256) {
            uint r = i / 128, c = i % 128;
            uint gr = tileRow + r, gc = kb + c;
            As[r][c] = (gr < M && gc < K) ? A[gr * K + gc] : 0;
        }
        for (uint i = lid; i < 128 * 128; i += 256) {
            uint r = i / 128, c = i % 128;
        ...
        for (uint kk = 0; kk < 128; kk += 8) {
            ...
            simdgroup_load(a0, &As[sr][kk], 129);
            simdgroup_load(a1, &As[sr + 8][kk], 129);
